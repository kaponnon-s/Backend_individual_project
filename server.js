...
getPost = async (req) => {
	return new Promise((resolve) => {
		...
		req
			...
			.on("end", () => {
                console.log(Buffer.concat(req_input).toString());
...
http
	.createServer(async function (req, res) {
		...
		switch (path.pathname) {
			...
			case "/pay":
				try {
					...
					if (
						...
						if (!new Validator(id).regMoney.test(id)) {
							response_object.message = `Id is not number.`;
						} else if (!new Validator(station_name).regAddress.test(station_name)) {
			...
			case "/checkToll":
				try {
					if (new Validator(path.query.name).regAddress.test(path.query.name)) {
